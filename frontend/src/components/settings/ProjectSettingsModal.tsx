...
export function ProjectSettingsModal({
  ...
  const [workspaceModesList, setWorkspaceModesList] = useState<
    WorkspaceModeInfo[]
  >([]);
  const [workspaceModesDir, setWorkspaceModesDir] = useState<{
    path: string;
    exists: boolean;
  } | null>(null);
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Workspace-Plugins konnten nicht geladen werden",
        );
      } finally {
  ...
  const loadAgents = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Agenten konnten nicht geladen werden",
      );
    } finally {
  ...
  const handleResetNavi = async () => {
    if (
      !window.confirm(
        "Alle Navi-Anpassungen (Anweisungen, Arbeitspläne) verwerfen und auf die Standard-States zurücksetzen?",
      )
    )
      return;
    setSavingConfig(true);
  ...
  const setAgentToolkitEnabled = (
    toolkitId: ChatToolkitId,
    enabled: boolean,
  ) => {
    setAgentForm((prev) => {
      if (!prev) return prev;
  ...
  const handleSaveAgent = async () => {
    ...
    const effectiveId =
      agentForm.editingId ??
      agentForm.id.trim().replace(/\s+/g, "-").toLowerCase();
    ...
    const preset: AgentPreset = {
      ...
      ...(agentForm.threadModeId.trim()
        ? { threadModeId: agentForm.threadModeId.trim() }
        : {}),
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Agent konnte nicht gespeichert werden",
      );
    } finally {
  ...
  const handleSaveLlm = async () => {
    ...
    if (!editingId) {
      if (hasFast && !fastApiKey.trim()) {
        setError(
          "Fast-API-Key ist für einen neuen Eintrag mit Fast-Modell erforderlich",
        );
        return;
      }
      if (hasReasoning && !hasFast && !reasoningApiKey.trim()) {
        setError(
          "Reasoning-API-Key ist erforderlich, wenn kein Fast-Modell angegeben ist",
        );
        return;
      }
    }
        ...
        {loading ? (
          ...
          <>
            ...
            {!initialized && tab !== "aiProviders" && (
              <div className="ps-uninit">
                <p className="ps-uninit-text">
                  This project has no <code>.assistant/</code> configuration
                  yet. Initialize it to create project-specific modes, rules,
                  and settings that will be committed with your project via Git.
                </p>
            ...
            {initialized && tab === "general" && (
              <div className="ps-tab-content">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setConfig((p) => ({ ...p, defaultMode: e.target.value }))
                  }
                >
                  <option value="">
                ...
                <p className="ps-hint">
                  Der Datei-Browser nutzt immer den Standard-Modus.
                  Medien-Projekte (Buch, Musik, …) legst du per Rechtsklick auf
                  einen Ordner an. Eigene Typen als YAML-Plugins findest du
                ...
                <TagListEditor
                  ...
                  onAdd={(v) =>
                    setConfig((p) => ({
                      ...p,
                      alwaysInclude: [...p.alwaysInclude, v],
                    }))
                  }
                  onRemove={(i) =>
                    setConfig((p) => ({
                      ...p,
                      alwaysInclude: p.alwaysInclude.filter(
                        (_, idx) => idx !== i,
                      ),
                    }))
                  }
                ...
                <p className="ps-hint">
                  Diese Regeln werden in jeden System-Prompt injiziert (ähnlich
                ...
                <p className="ps-hint">
                  Wie viele Tool-Aufruf-Runden (Suche, Datei lesen, …) die KI
                  maximal durchführen darf, bevor sie antwortet. Standard: 6.
                </p>
                <input
                  className="ps-input"
                  type="number"
                  ...
                  onChange={(e) => {
                    ...
                    setConfig((p) => ({
                      ...p,
                      maxToolRounds: isNaN(v) ? undefined : v,
                    }));
                  }}
                  style={{ width: "80px" }}
                />
                ...
                <p className="ps-hint">
                  LLM, das beim Klick auf "Zusammenfassung an Parent senden" im
                  Thread-Workspace genutzt wird. Leer lassen = erstes
                  konfiguriertes LLM.
                </p>
                <select
                  className="ps-input"
                  ...
                  onChange={(e) =>
                    setConfig((p) => ({
                      ...p,
                      threadSummaryLlmId: e.target.value,
                    }))
                  }
                ...
                <p className="ps-hint">
                  Optionale Funktionen, die standardmäßig aus sind und bei
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setConfig((p) => ({
                        ...
                        extraFeatures: {
                          ...p.extraFeatures,
                          chatDownload: e.target.checked,
                        },
                      }))
                    }
                  />
                  <label
                    htmlFor="extraChatDownload"
                    className="ps-toggle-label"
                  >
                    Chat-Verlauf als Datei herunterladen (Markdown-Export in der
                    Chat-Historie)
                  </label>
                </div>
            ...
            {initialized && tab === "quickChat" && (
              <div className="ps-tab-content">
                <p className="ps-hint">
                  Das schwebende <strong>Quick Chat</strong>-Fenster
                  (Tastenkürzel <kbd>Alt+E</kbd>) ist für kurze Fragen ohne
                  Projekt-Kontext: Begriffe, Formulierungen, Websuche. Es nutzt
                  kein Datei-Referenzsystem.
                </p>
                <label className="ps-label">LLM für Quick Chat</label>
                <p className="ps-hint">
                  Leer lassen = erstes konfiguriertes LLM aus der Liste.
                ...
                <select
                  ...
                  onChange={(e) =>
                    setConfig((p) => ({ ...p, quickChatLlmId: e.target.value }))
                  }
                  disabled={loadingLlms || !llmsState?.providers?.length}
                >
                  <option value="">Standard (erstes LLM)</option>
                ...
                <div className="ps-actions" style={{ marginTop: "1rem" }}>
                  <button
                    className="ps-save-btn"
                    onClick={() => void handleSaveQuickChatConfig()}
                    disabled={savingConfig}
                  >
                    {savingConfig ? (
                      <>
                  ...
                  <div className="ps-mode-form">
                    <div className="ps-form-nav">
                      ...
                      <span className="ps-form-title">
                        {editingModeId ? "Edit Mode" : "New Mode"}
                      </span>
                    </div>
                    ...
                    <input
                      ...
                      value={modeForm.id}
                      onChange={(e) =>
                        setModeForm((p) => p && { ...p, id: e.target.value })
                      }
                    ...
                    <input
                      ...
                      value={modeForm.name}
                      onChange={(e) =>
                        setModeForm((p) => p && { ...p, name: e.target.value })
                      }
                    ...
                    <p className="ps-hint">
                      „Nur Agenten“: Modus erscheint nicht im
                      Haupt-Chat-Modusmenü, bleibt aber für Agent-Vorlagen und
                      ...
                      onChange={(e) =>
                        setModeForm(
                          (p) =>
                            p && {
                              ...p,
                              agentOnly: e.target.value === "agent",
                            },
                        )
                      }
                    >
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setModeForm(
                          (p) => p && { ...p, systemPrompt: e.target.value },
                        )
                      }
                    ...
                    <label className="ps-label">
                      Auto-Include Files{" "}
                      <span className="ps-label-hint">(one per line)</span>
                    </label>
                    <textarea
                      ...
                      onChange={(e) =>
                        setModeForm(
                          (p) => p && { ...p, autoIncludes: e.target.value },
                        )
                      }
                    ...
                    <label className="ps-label">
                      LLM{" "}
                      <span className="ps-label-hint">
                        (optional — leer = globaler aktiver Eintrag)
                      </span>
                    </label>
                    <select
                      className="ps-input"
                      ...
                      onChange={(e) =>
                        setModeForm((p) => p && { ...p, llmId: e.target.value })
                      }
                    >
                    ...
                    <div className="ps-toggle-row">
                      <input
                        ...
                        onChange={(e) =>
                          setModeForm(
                            (p) =>
                              p && { ...p, useReasoning: e.target.checked },
                          )
                        }
                      />
                      <label
                        htmlFor="modeUseReasoning"
                        className="ps-toggle-label"
                      >
                ...
                {agentForm ? (
                  <div className="ps-mode-form">
                    ...
                    <input
                      ...
                      value={agentForm.id}
                      onChange={(e) =>
                        setAgentForm((p) => p && { ...p, id: e.target.value })
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAgentForm((p) => p && { ...p, name: e.target.value })
                      }
                    ...
                    <p className="ps-hint">
                      LLM und Kontext kommen aus dem gewählten Modus (Tab{" "}
                      <strong>Modes</strong>).
                    </p>
                    <select
                      className="ps-input"
                      ...
                      onChange={(e) => {
                        ...
                        const modeLlm = modes.find(
                          (m) => m.id === nextModeId,
                        )?.llmId;
                        const lp = modeLlm
                          ? (llmsState?.providers ?? []).find(
                              (x) => x.id === modeLlm,
                            )
                          : undefined;
                    ...
                    <label className="ps-label">
                      Modus für Threads{" "}
                      <span className="ps-label-hint">
                        (optional — leer = wie Eltern-Chat; nur wenn dieser Chat
                        die Vorlage nutzt)
                      </span>
                    </label>
                    <select
                      ...
                      onChange={(e) =>
                        setAgentForm(
                          (p) => p && { ...p, threadModeId: e.target.value },
                        )
                      }
                    >
                      ...
                      return (
                        <>
                          ...
                          <div className="ps-toggle-row">
                            ...
                            />
                            <label
                              htmlFor="agentPresetUseReasoning"
                              className="ps-toggle-label"
                            >
                    ...
                    <p className="ps-hint">
                      Aktiviert = Toolkit steht in geführten Chats mit dieser
                      ...
                      {CHAT_TOOLKIT_IDS.map((kitId) => (
                        <div key={kitId} className="ps-toggle-row">
                          <input
                            ...
                            id={`agentTk-${kitId}`}
                            checked={
                              !agentForm.disabledToolkits.includes(kitId)
                            }
                            onChange={(e) =>
                              setAgentToolkitEnabled(kitId, e.target.checked)
                            }
                          />
                          <label
                            htmlFor={`agentTk-${kitId}`}
                            className="ps-toggle-label"
                          >
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setAgentForm(
                          (p) =>
                            p && { ...p, initialSteeringPlan: e.target.value },
                        )
                      }
                    ...
                    <div className="ps-list">
                      {!loadingAgents && agents.length === 0 && (
                        <div className="ps-empty">
                          Keine Agent-Vorlagen. Lege eine an, um sie beim neuen
                          geführten Chat auszuwählen.
                        </div>
                      )}
                      {agents.map((a) => {
                        const modeOk = modes.some((m) => m.id === a.modeId);
                        const modeLabel =
                          modes.find((m) => m.id === a.modeId)?.name ??
                          a.modeId;
                        const threadModeLabel = a.threadModeId
                          ? (modes.find((m) => m.id === a.threadModeId)?.name ??
                            a.threadModeId)
                          : null;
                        const legacyThreadLlm = a.threadLlmId
                          ? ((llmsState?.providers ?? []).find(
                              (l) => l.id === a.threadLlmId,
                            )?.name ?? a.threadLlmId)
                          : null;
                        const rowTitle = `${a.id} · Modus: ${modeLabel}${
                          threadModeLabel
                            ? ` · Thread-Modus: ${threadModeLabel}`
                            : ""
            ...
            {initialized && tab === "workspacePlugins" && (
              <div className="ps-tab-content ps-wp-tab">
                ...
                {!workspaceModesDir?.exists && workspaceModesDir?.path && (
                  <p className="ps-hint ps-wp-missing">
                    Der Ordner existiert noch nicht — „Ordner öffnen“ legt ihn
                    an.
                  </p>
                )}
                ...
                <p className="ps-hint">
                  Eingebaut (classpath) und Benutzer (AppData). Überschreibt ein
                  User-YAML dieselbe id, zählt die Quelle als „user“.
                </p>
                    ...
                    <tbody>
                      ...
                      {workspaceModesList.map((m) => (
                        <tr key={m.id}>
                          ...
                          <td>
                            {m.source === "user" ? "user (AppData)" : "builtin"}
                          </td>
            ...
            {tab === "aiProviders" && (
              <div className="ps-tab-content">
                <p className="ps-hint">
                  Jeder Eintrag benötigt mindestens eine <strong>Fast</strong>-{" "}
                  <em>oder</em> eine <strong>⚡ Reasoning</strong>-Konfiguration
                  — jeweils mit eigenem API-URL, Key und Modell. Welcher Eintrag
                  genutzt wird, legst du pro Modus im Tab <em>Modes</em> fest.
                  Kein Eintrag zugewiesen? Der erste in der Liste wird als
                  Fallback verwendet. Bei leerer Liste greift der Server auf{" "}
                  <code>application.yml</code> / env zurück.
                </p>
                {llmForm ? (
                  <div className="ps-mode-form">
                    <div className="ps-form-nav">
                    ...
                    <input
                      ...
                      value={llmForm.name}
                      onChange={(e) =>
                        setLlmForm((p) => p && { ...p, name: e.target.value })
                      }
                      placeholder="z. B. eecc.ai"
                    />

                    <p className="ps-label ps-llm-section-header">
                      Fast-Konfiguration{" "}
                      <span className="ps-label-hint">
                        (optional, wenn Reasoning angegeben)
                      </span>
                    </p>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, fastApiUrl: e.target.value },
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, fastModel: e.target.value },
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, fastApiKey: e.target.value },
                        )
                      }
                      placeholder={
                        llmForm.editingId ? "(unverändert)" : "sk-..."
                      }
                      autoComplete="off"
                    />
                    ...
                    <p className="ps-label ps-llm-section-header">
                      ...
                      <span className="ps-label-hint">(optional)</span>
                    </p>
                    <p className="ps-hint">
                      Wenn leer, werden URL und Key der Fast-Konfiguration
                      verwendet.
                    </p>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, reasoningApiUrl: e.target.value },
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, reasoningModel: e.target.value },
                        )
                      }
                      placeholder="z. B. grok-4.20-0309-reasoning"
                    />
                    <label className="ps-label">
                      API key <span className="ps-label-hint">(optional)</span>
                    </label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, reasoningApiKey: e.target.value },
                        )
                      }
                    ...
                    <p className="ps-hint">
                      Geschätzter Token-Verbrauch wird im Chat als
                      Fortschrittsbalken angezeigt. Leer lassen = kein Limit.
                    </p>
                    <input
                      type="number"
                      className="ps-input"
                      ...
                      onChange={(e) =>
                        setLlmForm(
                          (p) => p && { ...p, maxTokens: e.target.value },
                        )
                      }
                  ...
                  <>
                    <div className="ps-list">
                      {(llmsState?.providers?.length ?? 0) === 0 && (
                        <div className="ps-empty">
                          Keine LLMs eingetragen — der Server nutzt
                          application.yml / AI_API_KEY.
                        </div>
                      )}
                      {llmsState?.providers?.map((p) => {
                          ...
                          >
                            ...
                            >
                              ...
                              {hasFast && !p.fastApiKeySet && (
                                <span className="ps-hint ps-ai-no-key">
                                  Kein Fast-Key gesetzt
                                </span>
                              )}
                              {!hasFast &&
                                hasReasoning &&
                                !p.reasoningApiKeySet && (
                                  <span className="ps-hint ps-ai-no-key">
                                    Kein Reasoning-Key gesetzt
                                  </span>
                                )}
                            </div>
