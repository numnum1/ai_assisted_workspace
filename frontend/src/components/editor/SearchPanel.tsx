  ...
  return (
    <div className="search-panel">
      <div className="search-panel-header">
        ...
        <button className="search-panel-close" onClick={onClose} title="Schließen">
          <X size={14} />
        </button>
      </div>
      ...
      <div className="search-panel-results">
        ...
        {!loading && Array.from(grouped.entries()).map(([path, fileHits]) => (
          <div key={path} className="search-result-group">
            ...
            >
              <FileText size={13} />
              <span className="search-result-file-name">{path.split('/').pop()}</span>
