...
- **Persistenz-Split:** Chat-Konversationen sind flüchtig/renderer-seitig (localStorage). Navi-**Konfiguration** (States, Tips, Persona, Use-Cases, Tools) ist dateibasiert persistent und liegt **pro Profil** unter `~/.writing-assistant/navi/profiles/<id>/*.json` (`naviStateConfigService.ts`, `naviProfileStore.ts`). Das Standardprofil „Marc" (`marc`) hat kein Verzeichnis, ist schreibgeschützt und entspricht exakt den `DEFAULT_NAVI_*`-Konstanten; Speichern darauf forkt automatisch in ein neues Profil. Profile sind per nativem Dialog exportier-/importierbar. Die Verbesserungs-LLM-Config (`improvement-llm.json`) bleibt global, da sie einen API-Key enthält.