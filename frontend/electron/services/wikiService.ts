...
/**
 * Write (create or overwrite) a wiki markdown file.
 * Path is relative to the wiki root (same semantics as readWikiFile).
 */
export async function writeWikiFile(
  ...
  return { path: normalizeRelativePath(relativeWikiPath), created };
}

/**
 * Perform an exact-string replacement inside a wiki markdown file.
 * Fails with a descriptive error if oldString does not appear exactly once
 * (so the AI can provide more context and retry).
 */
export async function patchWikiFile(
  ...
  if (occurrences === 0) {
    throw new Error(
      `wiki_patch: oldString not found in ${relativeWikiPath}. ` +
      `Make sure to copy the exact text (including whitespace) from wiki_read output.`,
    );
  }
  if (occurrences > 1) {
    throw new Error(
      `wiki_patch: oldString appears ${occurrences} times in ${relativeWikiPath}. ` +
  ...
  return { path: normalizeRelativePath(relativeWikiPath) };
}
