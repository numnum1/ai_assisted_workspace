...
async function readSubprojectType(directoryPath: string): Promise<string | null> {
  const markerPath = path.join(directoryPath, '.subproject.json');
  try {
    const raw = await fs.readFile(markerPath, 'utf8');
    const parsed = JSON.parse(raw) as { type?: unknown; workspaceMode?: unknown };
...
async function buildFileNode(projectRoot: string, currentPath: string): Promise<FileNode> {
...
export async function getTree(projectRoot: string | null): Promise<FileNode> {
  const root = ensureProjectRoot(projectRoot);
