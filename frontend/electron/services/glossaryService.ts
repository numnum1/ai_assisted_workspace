...
export interface GlossaryDeleteResult {
  status: string;
  deleted: boolean;
}

export interface GlossaryAddResult {
  status: string;
...
function getGlossaryPath(projectRoot: string | null): string {
...
async function readGlossaryFile(projectRoot: string | null): Promise<{
  exists: boolean;
  content: string;
}> {
...
export async function addGlossaryEntry(
  ...
  const existingIndex = entries.findIndex(
    (entry) => entry.term.localeCompare(normalizedTerm, undefined, { sensitivity: 'accent' }) === 0,
  );
...
export async function deleteGlossaryEntry(
  ...
  const nextEntries = entries.filter(
    (entry) =>
      entry.term.localeCompare(normalizedTerm, undefined, { sensitivity: 'accent' }) !== 0,
  );
